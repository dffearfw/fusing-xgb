...
def download_gldas_daily_swe(year):
    ...
    try:
        # GLDAS数据集信息
        dataset_id = "NASA/GLDAS/V021/NOAH/G025/T3H"  # 3小时数据
        swe_band = 'SWE_inst'  # 雪水当量波段
        ...
        # 加载GLDAS数据
        hourly_col = ee.ImageCollection(dataset_id) \
            .filterDate(start_date, end_date) \
            .select(swe_band)

        # 检查是否有数据
        size = hourly_col.size().getInfo()
        if size == 0:
            logger.warning(f"{year}年无可用数据，跳过")
            return None

        # 将3小时数据聚合为日平均值
        def create_daily_image(date):
            start = ee.Date(date)
            end = start.advance(1, 'day')
            daily = hourly_col.filterDate(start, end).mean()
            return daily.set('system:time_start', start.millis())

        # 创建日期序列
        start_obj = datetime(year, 1, 1)
        date_list = [start_obj + timedelta(days=d) for d in range(365 + (1 if year % 4 == 0 else 0))]
        daily_images = [create_daily_image(ee.Date(d.strftime("%Y-%m-%d"))) for d in date_list]

        # 创建影像集合
        daily_col = ee.ImageCollection(daily_images)

        # 转换为多波段影像（每日一个波段）
        daily_img = daily_col.toBands().clip(china_region)
...
# 5. 批量下载函数
def batch_download_gldas_swe(start_year=2003, end_year=2020):
    """分年下载GLDAS SWE数据"""
    # 检查进度文件
...
if __name__ == "__main__":
    ...
    if mode == "1":
        logger.info("开始下载任务...")
        task_records = batch_download_gldas_swe(2021, 2022)
